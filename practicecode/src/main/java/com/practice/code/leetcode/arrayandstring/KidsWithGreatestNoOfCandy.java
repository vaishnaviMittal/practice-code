...
public class KidsWithGreatestNoOfCandy {
	...
	// Example
	/*
	 * Example 1: Input: candies = [2,3,5,1,3], extraCandies = 3 Output:
	 ...
	 * 
	 * Example 3: Input: candies = [12,1,12], extraCandies = 10 Output:
	 * [true,false,true]
	 */

	// Constraint
	/*
	 * Constraints: n == candies.length 2 <= n <= 100 1 <= candies[i] <= 100 1 <=
	 * extraCandies <= 50
	 */
	...
	public Integer maxOfArray(int[] candies) {
		int max = 0;
		for (int i = 0; i < candies.length; i++) {
			if (candies[i] > max) {
				max = candies[i];
